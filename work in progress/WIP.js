 ...
 */
var chooseColor = function(e) {
	$("#chatbox-option-color").css("background", "#" + e).text("#" + e), $("#chatbox-input-color").val(e), $("#chatbox-messenger").css("color", "#" + e)
};
var convertHex = function(color) {
	var color = color.value,
		result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(color);
	result ? (document.getElementById('chatbox-color-r').value = parseInt(result[1], 16), document.getElementById('chatbox-color-g').value = parseInt(result[2], 16), document.getElementById('chatbox-color-b').value = parseInt(result[3], 16)) : null
};
var convertRGB = function(color) {
	var r = document.getElementById('chatbox-color-r').value,
		g = document.getElementById('chatbox-color-g').value,
		b = document.getElementById('chatbox-color-b').value,
		rgb = b | (g << 8) | (r << 16);
	document.getElementById('chatbox-color-hex').value = (0x1000000 + rgb).toString(16).slice(1);
};
$("#chatbox-option-color").click(function() {
	if (!document.getElementById('chatbox-color-inner')) {
		var c = '<table cellspacing="0" id="chatbox-color-inner">';
		...
		for (i = 5; i >= 0; i--) {
			...
			for (j = 5; j >= 0; j--) {
				for (k = 5; k >= 0; k--) {
					...
					c = c + '<td style="background: #' + col + '" title="#' + col + '" onclick="chooseColor(\'' + col + '\');"></td>'
				}
			}
			c = c + '</tr>'
		}
		$('#chatbox-color').append(c + '</table>');
	}
		if ($('#chatbox-messenger-form').css('height') == '70px') {
			$('#chatbox-messenger-form').animate({
				height: '200px'
			}, 200);
		} else {
			$('#chatbox-messenger-form').animate({
