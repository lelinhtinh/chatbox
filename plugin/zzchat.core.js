...
(function($) {
	var zzchat = {
		...
		core: {
			 ...
			 */
			copy_user_name: function(user_name) {
				$messenger[0].value += user_name;
				$messenger.focus();
				return false;
			},
			convert: {
				date: function(type, time) {
					switch (type) {
						/**
	...
	var newMessage = function(Messages) {
		if (Messages) {
			...
			$.each(arr, function(i, val) { // Duyệt qua từng tin
				...
				var mess_dateUTC = zzchat.core.convert.date('date', mess_date); // Thời gian gửi tin dạng UTC
